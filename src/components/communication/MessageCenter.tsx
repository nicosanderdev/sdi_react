...
const TABS_CONFIG = [
  ...
  { id: 'replied', label: 'Respondidos', icon: <CheckCircleIcon size={18} /> }, // Assuming a filter for this
...
export function MessageCenter() {
  ...
  const handleSelectMessage = useCallback(async (messageToList: Message) => {
    ...
    setSelectedMessage(null); // Clear previous detail first
  ...
  const handleSendReply = async () => {
    ...
    try {
      const replyData = {
        recipientId: selectedMessage.senderId || '', // This needs to be the ID of the original sender
      ...
      // This is tricky: senderId for reply might not be in selectedMessage if it's from the current user perspective
              ...
              {TABS_CONFIG.map(tab => (
                <button 
                  ...
                  className={`flex items-center justify-between w-full px-4 py-3 text-left transition-colors duration-150 ${
                    activeTab === tab.id ? 'bg-[#BEE9E8]/50 border-r-2 border-[#62B6CB]' : 'hover:bg-gray-100'
                  }`}
                >
                  ...
                  {tabCounts[tab.id] !== undefined && tabCounts[tab.id]! > 0 && (
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                        activeTab === tab.id ? 'bg-[#62B6CB] text-white' : 'bg-gray-200 text-gray-600'
                    }`}>
                      {tabCounts[tab.id]}
                    </span>
                  )}
                ...
                <div className="overflow-y-auto flex-grow">
                  {messages.map(message => (
                    <div 
                      ...
                      className={`p-4 border-b border-gray-200 cursor-pointer relative group transition-colors duration-150 ${
                        !message.isRead ? 'bg-[#E0F7FA]/60' : '' // Light cyan for unread
