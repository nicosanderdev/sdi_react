-- create_estate_property fixes:
-- - Quota subquery: "UserCompanies" -> "CompanyMembers" (20260331110000_admin_companies_management.sql).
-- - Plan limit: "Subscriptions" removed in flexible billing; use "MemberPlans" + "Plans"
--   (ListingLimit / MaxProperties), aligned with get_active_member_plan logic.
-- - RealEstateExtension."Category" is enum PropertyCategory; map RPC int 0..4 (see propertyTypeMapForward).
-- Drops the temporary compat view from an earlier revision if present.
-- insert_listing: see 20260430120000_insert_listing_rpc.sql (kept out of this file to preserve migration order).

begin;

drop view if exists public."UserCompanies";
...
begin
  ...
  begin
    ...
    -- Plan limit: flexible billing uses MemberPlans + Plans (Subscriptions table removed/legacy).
    ...
    left join lateral (
      ...
      where mp."MemberId" = v_member_id
        and mp."IsActive" = true
        and mp."StartDate" <= now()
