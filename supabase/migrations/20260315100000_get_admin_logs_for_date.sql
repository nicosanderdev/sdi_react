-- Admin logs by date: unified view of user actions, property moderation, and booking created/updated.
-- Apply this migration manually when ready. RLS applies (SECURITY INVOKER) so only admins see data.
...
AS $$
  ...
  SELECT
    ...
    COALESCE(ep."Title", '') AS target_display,
  ...
  JOIN "EstateProperties" ep ON ep."Id" = pma."PropertyId" AND ep."IsDeleted" = false
  LEFT JOIN "Members" m ON m."UserId" = pma."PerformedBy" AND m."IsDeleted" = false
...
COMMENT ON FUNCTION public.get_admin_logs_for_date(date) IS
  'Returns admin audit events for a given date: user actions (MemberActionHistory), property moderation (PropertyModerationActions), and booking created/updated. Caller must be admin (RLS on underlying tables).';
