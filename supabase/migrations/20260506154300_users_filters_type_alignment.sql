-- PROPOSAL ONLY: align subscription tier filtering with frontend enum values.
-- Fixes potential mismatch where Plans.Key is text but p_subscription_tier is integer.
...
begin
  ...
  computed as (
    select
      b.*,
      case
        when b.plan_key = 'free' then 0
        when b.plan_key = 'manager' then 1
        when b.plan_key = 'company_small' then 2
        when b.plan_key = 'company_unlimited' then 3
        when b.plan_key = 'manager_pro' then 4
        else 5
