-- Per-property amenity localized descriptions (en, es, pt) on EstatePropertyAmenity.
-- Manual apply by project owner (idempotent).
...
alter table public."EstatePropertyAmenity"
  add column if not exists "LocalizedDescriptions" jsonb not null default '{}'::jsonb;

comment on column public."EstatePropertyAmenity"."LocalizedDescriptions" is
  'Optional per-language copy for this amenity on this property. Keys: en, es, pt.';

create or replace function public.is_valid_amenity_localized_descriptions(p jsonb)
returns boolean
language sql
immutable
as $$
...
alter table public."EstatePropertyAmenity"
  drop constraint if exists estate_property_amenity_localized_descriptions_check;

alter table public."EstatePropertyAmenity"
  add constraint estate_property_amenity_localized_descriptions_check
  check (public.is_valid_amenity_localized_descriptions("LocalizedDescriptions"));

-- ---------------------------------------------------------------------------
-- 2) Sanitize descriptions object (strip empty / invalid keys)
-- ---------------------------------------------------------------------------
  ...
  select coalesce(
    (
      ...
      from (
        ...
        from jsonb_each(coalesce(p_descriptions, '{}'::jsonb))
...
-- ---------------------------------------------------------------------------
-- 3) Public amenities JSON builder
-- ---------------------------------------------------------------------------
...
comment on function public.build_property_amenities_json(uuid) is
  'Builds public amenity array for property detail RPCs (id, name, iconId, optional descriptions).';

-- ---------------------------------------------------------------------------
-- 4) create_estate_property — p_amenity_links support
-- ---------------------------------------------------------------------------
