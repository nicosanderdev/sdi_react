-- Admin property list: filter by offer kind (listing types + extensions), not listing status.
-- Apply manually in Supabase when ready. Replaces get_admin_properties_list signature:
--   removes p_status, adds p_offer_kind (real_estate | annual_rent | summer_rent | event_venue | null).
